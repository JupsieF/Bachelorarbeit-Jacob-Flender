...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!parse.success) {
      return NextResponse.json({ error: "Ungültige Daten", details: parse.error }, { status: 400 });
    }

    // 1. Pflegeprofil updaten
  ...
  .schema("bachelor_baseplant_jacob_flender")
    ...
    // 2. Wenn der Name geändert wurde, auch alle Plants mit diesem care_id updaten
    if (updates.name) {
